    ...
    , ui(new Ui::MainWindow)
{
    ...
    if (ui->statusbar && !m_clockLabel) {
        ...
        if (!m_clockLabel || !m_clockLeftSpacer || !m_clockRightSpacer) return;
...
{
    // The status filter combo is now owned by .ui.
    if (m_affStatusFilterCombo) return;
    m_affStatusFilterCombo = ui->affStatusFilterCombo;
    if (!m_affStatusFilterCombo) return;
...
{
    // The open-ended checkbox is now owned by .ui.
    if (!ui->affDateFinEdit) return;
    if (!m_affOpenEndedCheck)
        m_affOpenEndedCheck = ui->affOpenEndedCheck;
