...
void MainWindow::startFingerprintEnrollmentFromForm()
{
    ...
    if (ui->faceStatusLabel) {
        ui->faceStatusLabel->setText(tr("⏳ Enrôlement empreinte en cours... placez puis reposez le doigt."));
        ui->faceStatusLabel->setStyleSheet("color: #ef6c00; font-weight: bold;");
    }
...
bool MainWindow::saveFingerprintIdForEmployee(int employeeId, int fingerprintId) const
{
    ...
    // If schema only has EMPREINTE (BLOB), store the sensor ID as UTF-8 bytes.
    if (col == QStringLiteral("EMPREINTE")) {
        // Prefix marker helps distinguish sensor-id payload from any legacy binary data.
        q.bindValue(QStringLiteral(":fp"), QStringLiteral("FID:%1").arg(fingerprintId).toUtf8());
    } else {
        q.bindValue(QStringLiteral(":fp"), QString::number(fingerprintId));
    }
...
void MainWindow::rememberFingerprintMapping(int fingerprintId, int employeeId) const
{
    ...
    const QString iniPath = QDir(baseDir).filePath(QStringLiteral("fingerprint_map.ini"));
...
QString MainWindow::resolveEmployeeNameForFingerprintId(int fingerprintId) const
{
    int employeeId = -1;
    QString fullName;
    if (resolveEmployeeByFingerprintId(fingerprintId, employeeId, fullName)) {
        return fullName;
    }
    return QString();
}
...
bool MainWindow::resolveEmployeeByFingerprintId(int fingerprintId, int& employeeId, QString& fullName) const
{
    ...
    if (col == QStringLiteral("EMPREINTE")) {
        ...
        while (q.next()) {
            ...
            // Robust fallback for Oracle ODBC blob representations (possible trailing NULs).
            QString decoded = QString::fromUtf8(blob);
            decoded.remove(QChar('\0'));
            decoded = decoded.trimmed();

            if (decoded.startsWith(QStringLiteral("FID:"), Qt::CaseInsensitive)) {
                bool okId = false;
                const int decodedId = decoded.mid(4).trimmed().toInt(&okId);
                if (okId && decodedId == fingerprintId && id > 0) {
                    employeeId = id;
                    fullName = name;
                    return true;
                }
                ...
                bool okLegacy = false;
                const int decodedId = decoded.toInt(&okLegacy);
                if (okLegacy && decodedId == fingerprintId && id > 0) {
                    employeeId = id;
                    fullName = name;
                    return true;
                }
...
void MainWindow::processFingerprintTerminalLine(const QString& line)
{
    ...
    if (line.startsWith(QStringLiteral("MATCH:"))) {
        ...
        if (!resolveEmployeeByFingerprintId(fpId, employeeId, displayName) || employeeId <= 0) {
            ...
            if (ui->MainStacked && ui->MainStacked->currentIndex() == 0 && ui->faceStatusLabel) {
                ui->faceStatusLabel->setText(tr("Empreinte inconnue ou base non prête."));
                ui->faceStatusLabel->setStyleSheet("color:#c62828; font-weight:600;");
            }
            return;
        }
        ...
        if (ui->faceStatusLabel) {
            ui->faceStatusLabel->setText(tr("✔ Connexion par empreinte réussie"));
            ui->faceStatusLabel->setStyleSheet("color:#2e7d32; font-weight:700;");
        }
        return;
    }
    ...
    if (line == QStringLiteral("NO_MATCH")) {
        if (ui->MainStacked && ui->MainStacked->currentIndex() == 0 && ui->faceStatusLabel) {
            ui->faceStatusLabel->setText(tr("Empreinte non reconnue. Réessayez."));
            ui->faceStatusLabel->setStyleSheet("color:#ef6c00; font-weight:600;");
        }
        return;
    }
    ...
    if (line.startsWith(QStringLiteral("ENROLL_OK:"))) {
        ...
        if (!ok || fpId <= 0) {
            if (ui->faceStatusLabel) {
                ui->faceStatusLabel->setText(tr("❌ ID empreinte invalide reçu"));
                ui->faceStatusLabel->setStyleSheet("color: #c62828; font-weight: bold;");
            }
            return;
        }
        ...
        if (editingId > 0) {
            if (saveFingerprintIdForEmployee(editingId, fpId)) {
                if (ui->faceStatusLabel) {
                    ui->faceStatusLabel->setText(
                        tr("✔ Empreinte ID %1 enregistrée pour l'employé ID %2")
                            .arg(fpId)
                            .arg(editingId));
            ...
            if (ui->faceStatusLabel) {
                ui->faceStatusLabel->setText(
                    tr("✔ Empreinte capturée (ID %1). Enregistrez l'employé pour lier l'empreinte.")
                        .arg(fpId));
                ui->faceStatusLabel->setStyleSheet("color: #2e7d32; font-weight: bold;");
            }
        }
    ...
    if (line.startsWith(QStringLiteral("ENROLL_OK:")) ||
        line.startsWith(QStringLiteral("ENROLL_FAIL")) ||
...
void MainWindow::setupAffectationOpenEndedOption()
{
    ...
    // Adds a checkbox to toggle open-ended assignments (no end date).
...
void MainWindow::setupEmployeeFormValidation()
{
    ...
    if (!m_enrollFingerprintBtn && ui->formLayout) {
        m_enrollFingerprintBtn = new QPushButton(tr("Enrôler empreinte"), ui->ajoutpersonnel);
        m_enrollFingerprintBtn->setObjectName(QStringLiteral("enrollFingerprintBtn"));
        m_enrollFingerprintBtn->setToolTip(tr("Lancer l'enrôlement d'empreinte sur le terminal."));
        m_enrollFingerprintBtn->setMinimumHeight(34);
        ui->formLayout->addRow(tr("Empreinte"), m_enrollFingerprintBtn);
