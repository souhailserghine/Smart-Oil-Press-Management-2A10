...
void MainWindow::setupInteractiveHooks()
{
    if (ui->sidebar) {
        ...
        if (auto* headerLayout = ui->sidebar->findChild<QHBoxLayout*>(QStringLiteral("logoandnamesidebar"))) {
            auto* toggleBtn = ui->sidebar->findChild<QToolButton*>(QStringLiteral("sidebarToggleBtn"));
            if (!toggleBtn) {
                toggleBtn = new QToolButton(ui->sidebar);
...
void MainWindow::setupEmployeeFormValidation()
{
    ...
    if (!m_enrollFingerprintBtn) {
        m_enrollFingerprintBtn = ui->ajoutpersonnel
            ? ui->ajoutpersonnel->findChild<QPushButton*>(QStringLiteral("enrollFingerprintBtn"))
            : nullptr;
    }
    ...
    auto refreshFeedbackVisibility = [this]() {
        ...
        if (ui->module1) host = static_cast<QWidget*>(ui->module1);
        else if (ui->ajoutpersonnel) host = static_cast<QWidget*>(ui->ajoutpersonnel);
        else if (ui->formLayoutWidget) host = ui->formLayoutWidget->parentWidget();
        if (!host) return;
