...
void Citernes::creerSequenceMesure()
{
    ...
    q.exec(R"(
        BEGIN
            EXECUTE IMMEDIATE 'CREATE SEQUENCE MESURE_SEQ
    ...
    if (q.lastError().isValid())
        qDebug() << "[MESURE] creerSequenceMesure erreur :" << q.lastError().text();
}
...
void Citernes::sauvegarderMesure(int idCiterne, double niveauActuel, double temperature)
{
    ...
    q.prepare(
        ...
        "VALUES (MESURE_SEQ.NEXTVAL, :idCiterne, SYSDATE, :niveau, :temp)");
...
void Citernes::on_btnConnecterArduino_clicked()
{
    ...
    if (result != 0) result = m_arduino->connect_arduino();
...
void Citernes::onDonneesArduinoRecues()
{
    ...
    while (buffer.contains('\n')) {
        ...
        // ─── 1. Calcul des flags ──────────────────────────────────────────
        bool capteurActifMaintenant = (pourcentage > 0);
        bool frontMontant           = capteurActifMaintenant && !m_capteurActifPrecedent;
        bool niveauDebordement      = (pourcentage >= 90);

        // Mettre à jour l'état capteur pour le prochain cycle
        m_capteurActifPrecedent = capteurActifMaintenant;

        // Remettre le flag alerte à false dès que le niveau repasse sous 90%
        // → prêt pour un nouvel événement de débordement
        ...
        // ─── 3. Insertion BDD — front montant uniquement ──────────────────
        if (frontMontant) {
            sauvegarderMesure(idCit, pourcentage, temp);
            qDebug() << "[Arduino] ✅ Front montant → mesure insérée. Niveau :" << pourcentage << "%";
        } else if (capteurActifMaintenant) {
            qDebug() << "[Arduino] Capteur actif (" << pourcentage << "%) — pas de nouvelle insertion.";
        } else {
            qDebug() << "[Arduino] Capteur sec (0%) — prêt pour prochaine détection.";
        }
